...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const PRODUCTOS_KEY = `amc_productos_db_v1_${activeUserCode}`;
  const PRODUCTOS_LEGACY_KEY = 'amc_productos_db_v1';
  const MOVIMIENTOS_KEY = activeUserCode === '1110591592' ? 'amc_inventario_movimientos_v1' : `amc_inventario_movimientos_v1_${activeUserCode}`;
  ...
  const productos = () => { try { const raw = localStorage.getItem(PRODUCTOS_KEY) || (activeUserCode === '1110591592' ? localStorage.getItem(PRODUCTOS_LEGACY_KEY) : '[]'); const data = JSON.parse(raw || '[]'); return Array.isArray(data) ? data.filter(p => p.activo !== false) : []; } catch { return []; } };
  const normalizar = value => String(value || '').trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  const token = () => { try { return JSON.parse(sessionStorage.getItem(SESSION_KEY) || '{}').token || null; } catch { return null; } };
  async function cargarProductos() { const authToken = token(); if (authToken) { try { const response = await fetch(`${API_BASE}/productos?limit=1000&soloActivos=true`, { headers: { Authorization: `Bearer ${authToken}` } }); const data = await response.json(); if (!response.ok) throw new Error(data.message || 'No fue posible cargar el catálogo.'); state.productos = (data.productos || []).filter(p => p.activo !== false).map(p => ({ ...p, precioBase: p.precioBase ?? p.precio_base ?? 0, stockTotal: p.stockTotal ?? p.stock_total ?? 0 })); return; } catch (error) { console.warn('No fue posible cargar productos desde la base de datos; se usará el catálogo local.', error); } } state.productos = productos(); }
  const productoNombre = p => `${p.nombre || ''} · ${p.codigo || 'Sin código'}`;
  ...
  function agregarLinea() { state.lineas.push({ id: `L-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`, destino: 'INVENTARIO', productoId: '', detalle: '', cantidad: 1, unitario: 0, descuento: 0, iva: 19, retencion: 0 }); renderLineas(); actualizarResumen(); }
  ...
  function guardar() { state.lineas = state.lineas.map(linea => leerLinea(linea.id)); if (!state.tercero) return aviso('Selecciona el proveedor o tercero de la compra.', 'error'); if (!$('numero-proveedor').value.trim()) return aviso('Ingresa el prefijo o número de la factura del proveedor.', 'error'); if (!state.lineas.length || state.lineas.some(linea => !linea.productoId || linea.cantidad <= 0 || linea.unitario <= 0)) return aviso('Cada línea debe tener un producto o servicio activo seleccionado, cantidad y valor unitario.', 'error'); const totals = state.lineas.map(totalLinea).reduce((out, item) => Object.fromEntries(Object.keys(out).map(key => [key, out[key] + item[key]])), { base:0, descuento:0, iva:0, retencion:0, total:0 }); const next = consecutivo(); const compra = { id:`COMP-${activeUserCode}-${String(next).padStart(4,'0')}`, consecutivo:next, numeroCompra:`FC-${String(next).padStart(4,'0')}`, numeroProveedor:$('numero-proveedor').value.trim(), fecha:$('fecha-compra').value, tercero:{ nombre:state.tercero.nombre, documento:state.tercero.documento || '', email:state.tercero.email || '' }, lineas:state.lineas.map(linea => ({ ...linea, ...totalLinea(linea) })), totales:totals, observaciones:$('observaciones').value.trim(), generadoEn:new Date().toISOString(), estado:'REGISTRADA' }; const current = compras(); current.push(compra); localStorage.setItem(COMPRAS_KEY, JSON.stringify(current)); actualizarInventario(compra); aviso('Factura de compra registrada y el inventario fue actualizado.'); window.setTimeout(() => { window.location.href = 'facturas-compra-generadas.html'; }, 500); }
  function actualizarInventario(compra) { let key = activeUserCode === '1110591592' && localStorage.getItem(PRODUCTOS_LEGACY_KEY) ? PRODUCTOS_LEGACY_KEY : PRODUCTOS_KEY; let lista; try { lista = JSON.parse(localStorage.getItem(key) || '[]'); } catch { lista = []; } let movimientos; try { movimientos = JSON.parse(localStorage.getItem(MOVIMIENTOS_KEY) || '[]'); } catch { movimientos = []; } compra.lineas.filter(l => l.destino === 'INVENTARIO').forEach(linea => { const p = lista.find(item => String(item.id) === String(linea.productoId)); if (!p) return; const anterior = Number(p.stockTotal ?? p.stock_total ?? 0) || 0; const nuevo = anterior + linea.cantidad; if ('stock_total' in p) p.stock_total = nuevo; else p.stockTotal = nuevo; movimientos.push({ id:`COMPRA-${compra.id}-${linea.id}`, producto_id:p.id, tipo:'ENTRADA_COMPRA', cantidad:linea.cantidad, stock_anterior:anterior, stock_nuevo:nuevo, referencia:compra.numeroCompra, fecha:compra.fecha, creadoEn:compra.generadoEn }); }); localStorage.setItem(key, JSON.stringify(lista)); localStorage.setItem(MOVIMIENTOS_KEY, JSON.stringify(movimientos)); }
